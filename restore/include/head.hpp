...
/// @brief 恢复过程的各个模块。
///
/// 按时间顺序定义了一系列函数和方法，共同构成了一个恢复程序的框架。
...
/// @param inputFolder [out] 存储输入文件夹的路径。
/// @param outputFolder [out] 存储输出文件夹的路径。
/// @param overwrite bool [out] 是否应覆盖现有文件。
...
/// @param output_folder [in, out] 命令行传递的输出文件夹，在此函数中仅作`canonical`处理。
/// 
...
/// @param output_folder 从命令行传入，新目录将被创建的基本目录。
...
/// @param output_folder 目标文件夹的路径，文件将被复制到这里。
/// @param backuped_paths 备份元路径的列表。
