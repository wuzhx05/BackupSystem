...
///
/// @param ofs_directories [out] 用于写入目录信息的文件流。
/// @param ofs_file_info [out] 用于写入文件信息的文件流。
///
...
/// @brief 获取每个文件的信息（路径，修改时间，大小）并将其存储在 `file_infos` 中。
///
/// 该函数遍历文件路径列表，使用 `FileInfo` 构造函数检索其信息（路径，修改时间，大小），然后将此信息附加到 `file_infos` 中。执行过程中打印状态消息。
///
/// @param files [in] 表示文件路径。
/// @param file_infos [out] 对 `fileinfo::FileInfo` 对象的向量的引用，这些对象的信息将被存储。
...
///
/// @param ofs_file_info [in] 用于将文件信息写入JSON文件的输出流。
/// @param ofs_directories [in] 用于将目录路径写入JSON文件的输出流。
...
///
/// @param file_infos 对包含 `fileinfo::FileInfo` 对象的向量的引用，这些对象持有待检查文件的路径和其他元数据。
